    ...
    (function() {
      ...
      var runInContext = function runInContext2(context) {
        ...
        function baseNth(array, n) {
          ...
          n += n < 0 ? length2 : 0;
          return isIndex(n, length2) ? array[n] : undefined;
        }
        ...
        function createCurry(func, bitmask, arity) {
          ...
          function wrapper() {
            ...
            if (length2 < arity) {
              return createRecurry(func, bitmask, createHybrid, wrapper.placeholder, undefined, args, holders, undefined, undefined, arity - length2);
            }
            var fn = this && this !== root && this instanceof wrapper ? Ctor : func;
        ...
        function shuffleSelf(array, size2) {
          ...
          size2 = size2 === undefined ? length2 : size2;
          while (++index < size2) {
...
// src/modules/graphics-library/src/primitives/shared.ts
var LL_STRUCTURE_SIZE_BYTES = 128;
var LL_STRUCTURE_SIZE = LL_STRUCTURE_SIZE_BYTES / 4;

// src/modules/graphics-library/src/shared.ts
function BoundingBoxEmpty() {
  return {
    min: vec3_exports.fromValues(Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE),
    max: vec3_exports.fromValues(-Number.MAX_VALUE, -Number.MAX_VALUE, -Number.MAX_VALUE),
    center: vec3_exports.create(),
    primitive: 0
  };
}
function BoundingBoxClone(boundingBox) {
  return {
    min: vec3_exports.clone(boundingBox.min),
    max: vec3_exports.clone(boundingBox.max),
    center: vec3_exports.clone(boundingBox.center),
    primitive: boundingBox.primitive
  };
}
function BoundingBoxCalculateCenter(boundingBox) {
  boundingBox.center = vec3_exports.scale(vec3_exports.create(), vec3_exports.add(vec3_exports.create(), boundingBox.max, boundingBox.min), 0.5);
}
function BoundingBoxExtendByBox(boundingBox, extendBy) {
  boundingBox.min = vec3_exports.min(boundingBox.min, boundingBox.min, extendBy.min);
  boundingBox.max = vec3_exports.max(boundingBox.max, boundingBox.max, extendBy.max);
  BoundingBoxCalculateCenter(boundingBox);
  return boundingBox;
}
function BoundingBoxExtendByPoint(boundingBox, extendBy) {
  boundingBox.min = vec3_exports.min(boundingBox.min, boundingBox.min, extendBy);
  boundingBox.max = vec3_exports.max(boundingBox.max, boundingBox.max, extendBy);
  BoundingBoxCalculateCenter(boundingBox);
  return boundingBox;
}
function BoundingBoxDiagonal(boundingBox) {
  return vec3_exports.subtract(vec3_exports.create(), boundingBox.max, boundingBox.min);
}
function BoundingBoxHalfArea(boundingBox) {
  const d = BoundingBoxDiagonal(boundingBox);
  return (d[0] + d[1]) * d[2] + d[0] * d[1];
}
function BoundingBoxLargestAxis(boundingBox) {
  const d = BoundingBoxDiagonal(boundingBox);
  let axis = 0;
  if (d[0] < d[1])
    axis = 1;
...
// src/modules/graphics-library/src/primitives/direct_write.ts
function cylinderToBoundingBox(view, offset) {
  const result = BoundingBoxEmpty();
  const cylinderFrom = vec3_exports.fromValues(view.getFloat32(offset * LL_STRUCTURE_SIZE_BYTES + 0, true), view.getFloat32(offset * LL_STRUCTURE_SIZE_BYTES + 4, true), view.getFloat32(offset * LL_STRUCTURE_SIZE_BYTES + 8, true));
  const cylinderRadius = view.getFloat32(offset * LL_STRUCTURE_SIZE_BYTES + 12, true);
  const cylinderTo = vec3_exports.fromValues(view.getFloat32(offset * LL_STRUCTURE_SIZE_BYTES + 16, true), view.getFloat32(offset * LL_STRUCTURE_SIZE_BYTES + 20, true), view.getFloat32(offset * LL_STRUCTURE_SIZE_BYTES + 24, true));
  const a = vec3_exports.sub(vec3_exports.create(), cylinderTo, cylinderFrom);
  const d = 1 / vec3_exports.dot(a, a);
  const e = vec3_exports.create();
  e[0] = cylinderRadius * Math.sqrt(1 - a[0] * a[0] * d);
  e[1] = cylinderRadius * Math.sqrt(1 - a[1] * a[1] * d);
  e[2] = cylinderRadius * Math.sqrt(1 - a[2] * a[2] * d);
  result.min = vec3_exports.min(vec3_exports.create(), vec3_exports.sub(vec3_exports.create(), cylinderFrom, e), vec3_exports.sub(vec3_exports.create(), cylinderTo, e));
  result.max = vec3_exports.max(vec3_exports.create(), vec3_exports.add(vec3_exports.create(), cylinderFrom, e), vec3_exports.add(vec3_exports.create(), cylinderTo, e));
  return result;
}

// src/modules/graphics-library/src/primitives/sphere.ts
function sphereToBoundingBox(array, offset) {
  const result = BoundingBoxEmpty();
  const spherePosition = vec3_exports.fromValues(array.f32View[offset * LL_STRUCTURE_SIZE + 0], array.f32View[offset * LL_STRUCTURE_SIZE + 1], array.f32View[offset * LL_STRUCTURE_SIZE + 2]);
  const sphereRadius = array.f32View[offset * LL_STRUCTURE_SIZE + 3];
  BoundingBoxExtendByPoint(result, vec3_exports.add(vec3_exports.create(), spherePosition, vec3_exports.fromValues(sphereRadius, sphereRadius, sphereRadius)));
  BoundingBoxExtendByPoint(result, vec3_exports.add(vec3_exports.create(), spherePosition, vec3_exports.fromValues(-sphereRadius, -sphereRadius, -sphereRadius)));
  BoundingBoxCalculateCenter(result);
  return result;
}

// src/modules/graphics-library/src/primitives/rounded_cone.ts
function roundedConeToBoundingBox(array, offset) {
  const result = BoundingBoxEmpty();
  const cylinderFrom = vec3_exports.fromValues(array.f32View[offset * LL_STRUCTURE_SIZE + 0], array.f32View[offset * LL_STRUCTURE_SIZE + 1], array.f32View[offset * LL_STRUCTURE_SIZE + 2]);
  const cylinderRadius = array.f32View[offset * LL_STRUCTURE_SIZE + 3];
  const cylinderTo = vec3_exports.fromValues(array.f32View[offset * LL_STRUCTURE_SIZE + 4], array.f32View[offset * LL_STRUCTURE_SIZE + 5], array.f32View[offset * LL_STRUCTURE_SIZE + 6]);
  const a = vec3_exports.sub(vec3_exports.create(), cylinderTo, cylinderFrom);
  const directionExpand = vec3_exports.normalize(vec3_exports.create(), a);
  vec3_exports.scale(directionExpand, directionExpand, cylinderRadius * 1.1);
  vec3_exports.add(cylinderTo, cylinderTo, directionExpand);
  vec3_exports.negate(directionExpand, directionExpand);
  vec3_exports.add(cylinderFrom, cylinderFrom, directionExpand);
  const d = 1 / vec3_exports.dot(a, a);
  const e = vec3_exports.create();
  e[0] = cylinderRadius * Math.sqrt(1 - a[0] * a[0] * d);
  e[1] = cylinderRadius * Math.sqrt(1 - a[1] * a[1] * d);
...
// src/modules/graphics-library/src/primitives/quadratic_bezier.ts
function quadraticBezierToBoundingBox(array, offset) {
  const result = BoundingBoxEmpty();
  const p0 = vec3_exports.fromValues(array.f32View[offset * LL_STRUCTURE_SIZE + 0], array.f32View[offset * LL_STRUCTURE_SIZE + 1], array.f32View[offset * LL_STRUCTURE_SIZE + 2]);
  const p1 = vec3_exports.fromValues(array.f32View[offset * LL_STRUCTURE_SIZE + 4], array.f32View[offset * LL_STRUCTURE_SIZE + 5], array.f32View[offset * LL_STRUCTURE_SIZE + 6]);
  const p2 = vec3_exports.fromValues(array.f32View[offset * LL_STRUCTURE_SIZE + 8], array.f32View[offset * LL_STRUCTURE_SIZE + 9], array.f32View[offset * LL_STRUCTURE_SIZE + 10]);
  const radius = array.f32View[offset * LL_STRUCTURE_SIZE + 3];
  let mi = vec3_exports.min(vec3_exports.create(), p0, p2);
  let ma = vec3_exports.max(vec3_exports.create(), p0, p2);
  if (p1[0] < mi[0] || p1[0] > ma[0] || p1[1] < mi[1] || p1[1] > ma[1] || p1[2] < mi[2] || p1[2] > ma[2]) {
    const t = vec3_exports.create();
    const clamp = (num, min2, max2) => Math.min(Math.max(num, min2), max2);
    t[0] = clamp((p0[0] - p1[0]) / (p0[0] - 2 * p1[0] + p2[0]), 0, 1);
    t[1] = clamp((p0[1] - p1[1]) / (p0[1] - 2 * p1[1] + p2[1]), 0, 1);
    t[1] = clamp((p0[2] - p1[2]) / (p0[2] - 2 * p1[2] + p2[2]), 0, 1);
    const s = t.map((v) => 1 - v);
    const q = vec3_exports.create();
    q[0] = s[0] * s[0] * p0[0] + 2 * s[0] * t[0] * p1[0] + t[0] * t[0] * p2[0];
    q[1] = s[1] * s[1] * p0[1] + 2 * s[1] * t[1] * p1[1] + t[1] * t[1] * p2[1];
    q[2] = s[2] * s[2] * p0[2] + 2 * s[2] * t[2] * p1[2] + t[2] * t[2] * p2[2];
    mi = vec3_exports.min(vec3_exports.create(), mi, q);
    ma = vec3_exports.max(vec3_exports.create(), ma, q);
  }
  result.min = vec3_exports.add(vec3_exports.create(), mi, vec3_exports.fromValues(-radius, -radius, -radius));
  result.max = vec3_exports.add(vec3_exports.create(), ma, vec3_exports.fromValues(radius, radius, radius));
  BoundingBoxCalculateCenter(result);
  return result;
}

// src/modules/graphics-library/src/bvh/shared.ts
var NODE_SIZE = 12;
var NODE_SIZE_BYTES = NODE_SIZE * 4;
var BOUNDING_BOX_SIZE_BYTES = NODE_SIZE * 4;
function NodeEmpty() {
  return {
...
// src/modules/graphics-library/src/bvh/binned_sah_builder.worker.ts
...
var Bin = class {
  constructor() {
    this.bbox = BoundingBoxEmpty();
    this.primitiveCount = 0;
    this.rightCost = 0;
  }
};
function buildBVHBinnedSAH(bboxes, global_bbox) {
  ...
  const findSplit = (axis) => {
    ...
    current_count = 0;
    let best_split = [Number.MAX_VALUE, binCount];
  ...
  const build = (node, begin, end, depth) => {
    const makeLeaf = function(node2, begin2, end2) {
      node2.firstChildOrPrimitive = begin2;
      node2.primitiveCount = end2 - begin2;
    };
    ...
    const bestSplits = new Array(3);
    const bbox = node.boundingBox;
    const centerToBin = vec3_exports.scale(vec3_exports.create(), vec3_exports.inverse(vec3_exports.create(), BoundingBoxDiagonal(bbox)), binCount);
    const binOffset = vec3_exports.mul(vec3_exports.create(), bbox.min, vec3_exports.negate(vec3_exports.create(), centerToBin));
    const computeBinIndex = (center, axis) => {
    ...
    for (let i = begin; i < end; ++i) {
      const bbox2 = bboxes[i];
      for (let axis = 0; axis < 3; ++axis) {
        const bin = binsPerAxis[axis][computeBinIndex(bbox2.center, axis)];
        bin.primitiveCount += 1;
        BoundingBoxExtendByBox(bin.bbox, bbox2);
      }
    }
    for (let axis = 0; axis < 3; axis++) {
    ...
    const [leftSplit, rightSplit] = (0, import_lodash.partition)(indicesSlice, (bbox2) => {
      return computeBinIndex(bbox2.center, bestAxis) < splitIndex;
    });
...
ctx.onmessage = ({ data: { objectsBuffer } }) => {
  ...
  for (let i = 0; i < objectsCount; i++) {
    ...
    if (objectType != 9999 /* None */ && partOfBVH) {
      switch (objectType) {
        case 0 /* Sphere */:
          bboxes.push(sphereToBoundingBox(arrayViews, i));
          break;
        case 1 /* Cylinder */:
          bboxes.push(cylinderToBoundingBox(objectsBuffer, i));
          break;
        case 4 /* RoundedCone */:
          bboxes.push(roundedConeToBoundingBox(arrayViews, i));
          break;
        case 2 /* QuadraticBezierCurve */:
          bboxes.push(quadraticBezierToBoundingBox(arrayViews, i));
          break;
        default:
          continue;
      }
